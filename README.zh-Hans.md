...
可参考 [`.env.example`](.env.example)，通过同目录的 `.env` 调整镜像、管理员账号、扫描深度、Cookie 安全选项和容器内路径。Shell 环境变量优先于 `.env`；修改后执行 `docker compose up -d` 使配置生效。路径变量不会改变宿主机挂载目录，详见 [Compose 配置](docs/operations/docker.md#configure-with-env)。
