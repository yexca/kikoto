...
- `baseline/032_v0.5.0.sql` is the packaged fresh-install snapshot for schema
  032 and remains in use for v0.5.4 because no numbered SQL changed. Existing
  databases must continue through the numbered chain.
- `VERSION` currently reports v0.5.4; add migration `033` for the next schema
...
- A release with no new numbered SQL migration reuses the highest packaged
  ...
  `baseline/032_v0.5.4.sql`; generate a new baseline only when the numbered
