...
const latestNumberedMigrationVersion = 39
...
func TestMigrateFreshDatabaseReusesBaselineAcrossAppReleases(t *testing.T) {
	...
	if filename != "baseline/039_v0.6.1.sql" {
...
func TestMigrateUpgradesExistingDatabaseThroughNumberedChain(t *testing.T) {
	...
	if filename != "039_reconfigure_follow_triggers.sql" {
		t.Fatalf("applied migration = %q, want 039_reconfigure_follow_triggers.sql", filename)
	}
}
...
func TestMigratePreservesMislabeledV060Baseline(t *testing.T) {
	...
	if count != 6 || filename != "baseline/034_v0.5.5.sql" || checksum != originalChecksum || baselineChecksum != originalChecksum || value != `"retained"` {
...
func TestMigrateUpgradesRetiredBaselineLedger(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		},
		{
			...
			previousVersion: 32,
			wantHistory:     "baseline/032_current.sql,033_metadata_sync_issues.sql,034_user_preferences.sql,035_remove_custom_workflow_definitions.sql,036_work_search_index.sql,037_list_and_foreign_key_indexes.sql,038_metadata_snapshot_card_summary.sql,039_reconfigure_follow_triggers.sql",
		},
	} {
