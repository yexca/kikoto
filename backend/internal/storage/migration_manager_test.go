...
const latestNumberedMigrationVersion = 35
...
func TestMigrateFreshDatabaseReusesBaselineAcrossAppReleases(t *testing.T) {
	...
	if filename != "baseline/035_v0.6.1.sql" {
...
func TestMigrateUpgradesExistingDatabaseThroughNumberedChain(t *testing.T) {
	...
	if filename != "035_remove_custom_workflow_definitions.sql" {
		t.Fatalf("applied migration = %q, want 035_remove_custom_workflow_definitions.sql", filename)
	}
}
...
func TestMigratePreservesMislabeledV060Baseline(t *testing.T) {
	...
	if count != 2 || filename != "baseline/034_v0.5.5.sql" || checksum != originalChecksum || baselineChecksum != originalChecksum || value != `"retained"` {
...
func TestMigrateUpgradesRetiredBaselineLedger(t *testing.T) {
	...
	for _, testCase := range []struct {
	...
	}{
		{
			...
			previousVersion: 31,
			wantHistory:     "baseline/031_current.sql,032_shared_availability_watch.sql,033_metadata_sync_issues.sql,034_user_preferences.sql,035_remove_custom_workflow_definitions.sql",
		},
		{
			name:            "schema version 032 upgrades without replaying old migrations",
			baseline:        "baseline/032_current.sql",
			previousVersion: 32,
			wantHistory:     "baseline/032_current.sql,033_metadata_sync_issues.sql,034_user_preferences.sql,035_remove_custom_workflow_definitions.sql",
		},
	} {
