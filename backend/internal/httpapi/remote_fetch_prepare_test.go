...
func TestRemoteFetchMetadataReadyAcceptsNonJapaneseOriginWhenLocaleMatches(t *testing.T) {
	db := openMigratedTestDB(t)
	server := NewServer(db, config.Config{})
	result, err := db.Exec("INSERT INTO work (primary_code, title) VALUES ('RJ00000002', 'English title')")
	if err != nil {
		t.Fatal(err)
	}
	workID, err := result.LastInsertId()
	...
	logical, err := db.Exec("INSERT INTO logical_work (canonical_work_id, canonical_code) VALUES (?, 'RJ00000002')", workID)
	if err != nil {
		t.Fatal(err)
	}
	logicalID, err := logical.LastInsertId()
	...
	if _, err := db.Exec(`
		INSERT INTO work_edition (work_id, logical_work_id, provider_id, primary_code, metadata_language, edition_label, is_canonical, translation_kind)
		VALUES (?, ?, ?, 'RJ00000002', 'ENG', 'English', 1, 'origin')
	`, workID, logicalID, providerID); err != nil {
		t.Fatal(err)
	}
	if _, err := db.Exec(`
		INSERT INTO dlsite_metadata_variant (logical_work_id, work_id, provider_id, external_id, edition_language, request_locale, title, tags_json)
		VALUES (?, ?, ?, 'RJ00000002', 'ENG', 'en-us', 'English title', '[]')
	`, logicalID, workID, providerID); err != nil {
		t.Fatal(err)
	}

	ready, err := server.remoteFetchMetadataReady(context.Background(), "RJ00000002")
	if err != nil {
		t.Fatal(err)
	}
	if !ready {
		t.Fatal("matching non-Japanese origin variant was treated as incomplete")
	}
}
