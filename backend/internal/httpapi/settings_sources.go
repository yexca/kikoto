...
func (s *Server) listRemoteSourceWorks(w http.ResponseWriter, r *http.Request) {
	...
	familyCodes, err := s.remoteSearchFamilyCodes(r.Context(), r.URL.Query().Get("q"))
	if err != nil {
		writeError(w, err)
		return
	}
	if len(familyCodes) > 1 {
		works, total, sortApplied, err := s.remoteFamilyCodePage(
			r.Context(), userID, source.ID, client, familyCodes, upstreamOrder, direction,
...
func (s *Server) remoteSearchFamilyCodes(ctx context.Context, query string) ([]string, error) {
	clauses := parseListSearchClauses(query)
	if len(clauses) != 1 || clauses[0].Kind != "code" {
		return nil, nil
	}
	requested := normalizeDLsiteCode(clauses[0].Value)
	if requested == "" {
		return nil, nil
	}
	rows, err := s.db.QueryContext(ctx, `
		WITH matched_logical_work AS (
		...
		SELECT primary_code
		FROM work_edition
		WHERE logical_work_id IN (SELECT logical_work_id FROM matched_logical_work)
		UNION
		SELECT primary_code
		FROM work_code_alias
		WHERE logical_work_id IN (SELECT logical_work_id FROM matched_logical_work)
		...
		FROM logical_work
		WHERE id IN (SELECT logical_work_id FROM matched_logical_work)
	`, requested, requested)
	if err != nil {
		return nil, err
	}
	...
	addCandidate := func(value string) {
		code := normalizeDLsiteCode(value)
		if code == "" || seen[code] {
			return
		}
		seen[code] = true
	...
	if len(candidates) == 1 && s.remoteEditionLookup != nil {
		if product, lookupErr := s.remoteEditionLookup.FetchProduct(ctx, requested); lookupErr == nil {
			addCandidate(product.WorkNo)
			addCandidate(product.ProductID)
			for _, edition := range product.LanguageEditions {
				addCandidate(edition.WorkNo)
			}
		}
	}
	if len(candidates) > 1 {
	...
	const maxRemoteFamilyCodes = 16
	if len(candidates) > maxRemoteFamilyCodes {
		candidates = candidates[:maxRemoteFamilyCodes]
	}
	return candidates, nil
}

func (s *Server) remoteFamilyCodePage(
	ctx context.Context,
	userID int64,
	sourceID int64,
	client *kikoeru.Client,
	...
	for _, code := range codes {
		if normalized := normalizeDLsiteCode(code); normalized != "" {
			allowed[normalized] = true
		}
	}
	remoteWorks := []kikoeru.Work{}
	seen := map[string]bool{}
	sortApplied := true
	for _, code := range codes {
		var result kikoeru.WorksPage
		var err error
		if s.cfg.IsDemo() {
			result, err = client.SearchWorksSortedSeeded(ctx, 1, 100, code, order, direction, seed)
		} else {
			result, err = client.ListWorksSortedSeeded(ctx, 1, 100, code, order, direction, seed)
		}
		if err != nil {
			return nil, 0, false, err
		}
		...
		for _, work := range result.Works {
			remoteCode := normalizeDLsiteCode(normalizedRemoteWorkCode(work))
			if !allowed[remoteCode] {
				continue
			}
			key := remoteCode + ":" + strconv.FormatInt(work.ID, 10)
			if seen[key] {
				continue
			}
			seen[key] = true
			remoteWorks = append(remoteWorks, work)
		}
	}
	summaries, err := s.remoteWorkSummaries(ctx, userID, sourceID, remoteWorks, language, includeRecommendation)
	if err != nil {
		return nil, 0, false, err
	}
	total := len(summaries)
	...
	return summaries[start:end], total, sortApplied, nil
}
