...
func TestApplyRemoteFetchSourceDecisionSelectsPerFileSource(t *testing.T) {
	...
	if err := applyRemoteFetchSourceDecision(&item, remoteFetchFileDecision{SourceID: 2}, options, remoteSourceForUse{ID: 1}, "RJ00000000"); err != nil {
		t.Fatal(err)
	}
	if item.RemoteSourceID != 2 || item.RemoteSourceCode != "two" || item.SourcePath != "https://two.invalid/track" {
		t.Fatalf("selected item = %+v", item)
	}
	if item.CachePath != "media/two/RJ/000/RJ00000000/audio/track.mp3" {
...
func TestKeepBothAllocatesSafeTarget(t *testing.T) {
	...
	existing := filepath.Join(dataRoot, "source", "RJ00000000", "track.mp3")
	...
	item := remoteWorkSavePlanItem{
		Path: "track.mp3", TargetPath: "source/RJ00000000/track.mp3", RemoteSourceCode: "remote_two",
		TargetConflict: true, TargetConflictReason: "different size", Action: "conflict",
	}
	seen := map[string]string{item.TargetPath: item.Path}
	if err := server.applyRemoteFetchConflictDecision(&item, remoteFetchFileDecision{Resolution: "keep_both"}, "source/RJ00000000", seen); err != nil {
		t.Fatal(err)
	}
	if item.TargetConflict || item.Action != "" || item.TargetPath != "source/RJ00000000/track (remote_two).mp3" {
...
func TestFinishFetchPresenceKeepsSourceWithoutCreatingTracked(t *testing.T) {
	...
	statements := []string{
		...
		`INSERT INTO media_file_location (media_item_id, file_source_id, location_type, path, availability) VALUES (1, 1, 'remote_stream', 'track.mp3', 'available'), (1, 1, 'cache', 'media/remote/RJ/000/RJ00000000/track.mp3', 'available')`,
	}
	...
	if err := server.finishFetchPresence(ctx, 1, []int64{1}, 2, "RJ00000000"); err != nil {
		t.Fatal(err)
	}
