...
func newUserHandlerFixture(t *testing.T) userHandlerFixture {
	...
	root, err := server.accountStore.CreateInitialAdministrator(context.Background(), "root", "synthetic-password")
	if err != nil {
		t.Fatal(err)
	}
...
func TestUserHandlersManageLifecycleAndProtectSuperAdministrators(t *testing.T) {
	...
	if lastSuperResponse.Code != http.StatusBadRequest || !strings.Contains(lastSuperResponse.Body.String(), "at least one enabled super administrator") {
		t.Fatalf("last super administrator disable status = %d, body = %s", lastSuperResponse.Code, lastSuperResponse.Body.String())
	}
...
func TestInitialAdministratorIsManagedLikeOtherSuperAdministrators(t *testing.T) {
	fixture := newUserHandlerFixture(t)
	...
	updateResponse := httptest.NewRecorder()
	updateRequest := userHandlerRequest(http.MethodPatch, "/api/users/"+rootID, `{"password":"synthetic-password-2","role":"admin"}`, superActor)
	updateRequest.SetPathValue("id", rootID)
	fixture.server.updateUser(updateResponse, updateRequest)
	if updateResponse.Code != http.StatusOK {
		t.Fatalf("initial administrator update status = %d, body = %s", updateResponse.Code, updateResponse.Body.String())
	}
	if _, err := fixture.server.accountStore.Authenticate(context.Background(), "root", "synthetic-password-2", time.Now()); err != nil {
		t.Fatalf("initial administrator password was not replaced: %v", err)
	}
	...
	fixture.server.deleteUser(deleteResponse, deleteRequest)
	if deleteResponse.Code != http.StatusOK {
		t.Fatalf("initial administrator delete status = %d, body = %s", deleteResponse.Code, deleteResponse.Body.String())
	}
}
