...
func (s *Server) autoRefreshVoiceCatalog(w http.ResponseWriter, r *http.Request) {
	...
	s.refreshVoiceCatalogResponse(w, r, false)
}
...
func (s *Server) refreshVoiceCatalog(w http.ResponseWriter, r *http.Request) {
	...
	s.refreshVoiceCatalogResponse(w, r, true)
}

func (s *Server) refreshVoiceCatalogResponse(w http.ResponseWriter, r *http.Request, force bool) {
	...
	state, err := s.ensureVoiceCatalogRefresh(r.Context(), personID, force)
	if err != nil {
...
func (s *Server) ensureVoiceCatalogRefresh(ctx context.Context, personID int64, force bool) (voiceCatalogRefreshState, error) {
	...
	return s.enqueueVoiceCatalogRefresh(ctx, personID, queries, state.Generation+1, reason)
}
...
func (s *Server) enqueueVoiceCatalogRefresh(ctx context.Context, personID int64, queries []string, generation int64, reason string) (voiceCatalogRefreshState, error) {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return voiceCatalogRefreshState{}, err
	}
	defer func() { _ = tx.Rollback() }()
	definitionID, err := workflow.EnsureDefinition(ctx, tx, voiceCatalogRefreshWorkflow, "Refresh voice catalog", "Discover a voice actor's complete remote catalog and persist source observations before metadata follow-up.", voiceCatalogWorkflowDefinition())
	if err != nil {
		return voiceCatalogRefreshState{}, err
	}
	payload := voiceCatalogRefreshPayload{PersonID: personID, Queries: queries, Generation: generation}
	runID, err := workflow.InsertRun(ctx, tx, definitionID, voiceCatalogRefreshWorkflow, "Refresh voice catalog", "queued", "detail_view", reason, payload, map[string]any{
		"person_id": personID, "generation": generation, "queries": queries,
	})
	if err != nil {
		return voiceCatalogRefreshState{}, err
	}
	if _, err := workflow.InsertNodeRun(ctx, tx, runID, workflow.NodeRunSpec{
		...
		Status: "succeeded", Input: map[string]any{"person_id": personID}, Output: map[string]any{"queries": queries},
	}); err != nil {
		return voiceCatalogRefreshState{}, err
	}
	discoverNodeID, err := workflow.InsertNodeRun(ctx, tx, runID, workflow.NodeRunSpec{
		...
		Status: "queued", Input: map[string]any{"queries": queries, "page_size": voiceRemotePageSize},
	})
	if err != nil {
		return voiceCatalogRefreshState{}, err
	}
	for _, node := range []workflow.NodeRunSpec{
		{NodeID: "persist", NodeType: "persist_voice_catalog", DisplayName: "Persist voice catalog", Position: 3, Status: "queued"},
		{NodeID: "metadata", NodeType: "queue_metadata_sync", DisplayName: "Queue known-work metadata", Position: 4, Status: "queued"},
	} {
	...
	_, err = workflow.InsertJob(ctx, tx, runID, workflow.JobSpec{
		NodeRunID: discoverNodeID, WorkerType: voiceCatalogRefreshWorker, Status: "queued",
		Priority: workflow.JobPriorityBackground, ResourceKey: "remote:voice-catalog", Payload: payload,
		Checkpoint: map[string]any{"phase": "queued", "generation": generation}, Recoverable: true, MaxRetries: 3,
	})
	if err != nil {
		return voiceCatalogRefreshState{}, err
	}
	...
	`, personID, generation, string(queryJSON), runID); err != nil {
		return voiceCatalogRefreshState{}, err
	}
	if err := tx.Commit(); err != nil {
		return voiceCatalogRefreshState{}, err
	}
	return voiceCatalogRefreshState{
		Status: "queued", Reason: reason, LastStatus: "queued", Generation: generation, RunID: runID,
		Queries: queries, Sources: []voiceCatalogSourceStatus{}, LastAttemptAt: time.Now().UTC().Format(time.RFC3339),
	}, nil
}
...
func voiceCatalogWorkflowDefinition() map[string]any {
	return map[string]any{"nodes": []map[string]string{
		...
		{"id": "metadata", "type": "queue_metadata_sync", "displayName": "Queue known-work metadata"},
	}}
}
...
func (s *Server) executeVoiceCatalogRefreshJob(ctx context.Context, job workflowJobRecord) (runErr error) {
	...
	if payload.PersonID <= 0 || payload.Generation <= 0 || len(payload.Queries) == 0 {
	...
	_ = s.updateWorkflowJobCheckpoint(ctx, job.ID, "discovering", map[string]any{
		"personId": payload.PersonID, "sources": len(sources), "queries": len(payload.Queries),
	}, 0, len(sources))
	...
	for index, source := range sources {
		...
		go func(index int, source remoteSourceForUse) {
			...
			results[index] = s.discoverVoiceCatalogSource(ctx, job.RunID, source, payload.Queries, projector)
		}(index, source)
	}
	wait.Wait()
	...
	sourceStatuses := make([]voiceCatalogSourceStatus, 0, len(results))
	pagesFetched := 0
	eligibleSources := 0
	successfulSources := 0
	for index, result := range results {
		...
		if activeErr != nil {
			_ = s.failClaimedWorkflowJob(ctx, job, activeErr.Error())
			return activeErr
		}
		if !active {
			return nil
		...
		if result.Complete {
			successfulSources++
			workIDs, persistErr := s.persistVoiceCatalogSource(ctx, payload.PersonID, payload.Generation, result)
			if persistErr != nil {
		...
		sourceStatuses = append(sourceStatuses, result.Status)
	...
	catalogComplete := successfulSources == eligibleSources
	metadataQueued := 0
	metadataFailures := 0
	...
	for _, workID := range workIDs {
		...
		metadataRun, queueErr := s.enqueueWorkMetadataSync(context.WithoutCancel(ctx), workID)
		if queueErr != nil {
			slog.Warn("queue voice catalog metadata sync", "run_id", job.RunID, "work_id", workID, "error", queueErr)
			metadataFailures++
			continue
		}
		if metadataRun.RunID > 0 {
			metadataQueued++
		}
	}
	if metadataFailures > 0 && status == "succeeded" {
		status = "partial"
	}
	active, err = s.voiceCatalogRefreshRunActive(ctx, payload, job.RunID)
	if err != nil {
	...
	if err := s.finishVoiceCatalogRefreshJob(ctx, job, nodeIDs, payload, status, catalogComplete, sourceStatuses, pagesFetched, catalogWorks, metadataQueued, metadataFailures); err != nil {
		_ = s.failClaimedWorkflowJob(ctx, job, err.Error())
		return err
	}
	if status == "failed" {
...
func (s *Server) voiceCatalogRefreshRunActive(ctx context.Context, payload voiceCatalogRefreshPayload, runID int64) (bool, error) {
	...
	if status == "cancelled" {
		_, err := s.db.ExecContext(context.WithoutCancel(ctx), `
			...
			WHERE person_id = ? AND generation = ?
		`, runID, payload.PersonID, payload.Generation)
		return false, err
	}
...
func (s *Server) discoverVoiceCatalogSource(ctx context.Context, runID int64, source remoteSourceForUse, queries []string, projector remoteCatalogProjector) voiceCatalogSourceResult {
	...
	for _, query := range queries {
		...
		for pageNumber := 1; ; pageNumber++ {
			...
			page, err := client.ListWorks(sourceCtx, pageNumber, voiceRemotePageSize, keyword)
			if err != nil {
				result.Err = err
...
func (s *Server) persistVoiceCatalogSource(ctx context.Context, personID int64, generation int64, result voiceCatalogSourceResult) ([]int64, error) {
	if !result.Complete {
		return []int64{}, nil
	}
...
func (s *Server) finishVoiceCatalogRefreshJob(
	...
	lastError := ""
	if status == "failed" {
		lastError = "Remote source catalog refresh failed."
	} else if status == "partial" {
		lastError = "Some remote sources could not be refreshed."
	}
	summary := map[string]any{
		"person_id": payload.PersonID, "generation": payload.Generation, "queries": payload.Queries,
		"status": status, "sources": sourceStatuses, "pages_fetched": pagesFetched,
		"catalog_works": catalogWorks, "metadata_queued": metadataQueued, "metadata_queue_failures": metadataFailures,
	}
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	...
	if _, err := tx.ExecContext(ctx, `
		...
		SET source_status_json = ?,
			...
			updated_at = CURRENT_TIMESTAMP
		WHERE person_id = ? AND generation = ?
	`, string(sourceJSON), complete, time.Now().UTC().Format(time.RFC3339), status, job.RunID, lastError,
		complete, pagesFetched, catalogWorks, metadataQueued, payload.PersonID, payload.Generation); err != nil {
		return err
	}
	if _, err := tx.ExecContext(ctx, `
		...
		WHERE id = ?
	`, status, mustJSON(map[string]any{"sources": sourceStatuses, "pages_fetched": pagesFetched}), lastError, nodeIDs["discover"]); err != nil {
		return err
	}
	...
	if _, err := tx.ExecContext(ctx, `
		UPDATE workflow_node_run
		SET status = ?, output_json = ?, error_message = '', started_at = COALESCE(started_at, CURRENT_TIMESTAMP), finished_at = CURRENT_TIMESTAMP
		WHERE id = ?
	`, persistStatus, mustJSON(map[string]any{"catalog_works": catalogWorks, "generation": payload.Generation}), nodeIDs["persist"]); err != nil {
		return err
	}
	...
	if _, err := tx.ExecContext(ctx, `
		...
		SET status = ?, output_json = ?, error_message = '', started_at = COALESCE(started_at, CURRENT_TIMESTAMP), finished_at = CURRENT_TIMESTAMP
		WHERE id = ?
	`, metadataStatus, mustJSON(map[string]any{"queued": metadataQueued, "failures": metadataFailures}), nodeIDs["metadata"]); err != nil {
		return err
	}
	jobStatus := "succeeded"
	if status == "failed" {
	...
	if _, err := tx.ExecContext(ctx, `
		...
		WHERE id = ?
	`, jobStatus, len(sourceStatuses), len(sourceStatuses), lastError,
		mustJSON(map[string]any{"phase": "completed", "detail": summary, "progressCurrent": len(sourceStatuses), "progressTotal": len(sourceStatuses)}), job.ID); err != nil {
		return err
	}
	if _, err := tx.ExecContext(ctx, `
	...
	if err := workflow.InsertEvent(ctx, tx, job.RunID, workflow.EventSpec{
		NodeRunID: nodeIDs["discover"], JobID: job.ID, Level: eventLevelForWorkflowStatus(status),
