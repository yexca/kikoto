...
Workflows 右上の Activity はデスクトップのパネルまたはモバイルのシートを開きます。実行中の処理を上部に表示し、その下を **Needs attention** と **History** に分けます。未解決の候補、メタデータ問題、未確認の失敗が要対応となります。記録されたメタデータ問題の解消で通知を解除し、その他の失敗はユーザーごとに確認できます。History は元の結果を保持します。作品管理には問題対応とメタデータ設定を置き、同期は既存ワークフローへのショートカットから開きます。
