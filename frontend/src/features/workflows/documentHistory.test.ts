...
describe("workflow document history", () => {
  it("coalesces continuous inspector edits and clears redo after divergence", () => {
    ...
    state = workflowDocumentHistoryReducer(state, {
      type: "change",
      changedAt: 100,
      group: "name",
      update: { ...state.present, command: { enabled: true, alias: "a" } },
    });
    state = workflowDocumentHistoryReducer(state, {
      type: "change",
      changedAt: 200,
      group: "name",
      update: { ...state.present, command: { enabled: true, alias: "ab" } },
    });
    ...
    state = workflowDocumentHistoryReducer(state, {
      type: "change",
      changedAt: 1000,
      update: { ...state.present, policy: { requirePreview: false } },
    });
